...
class MetaPathEmbedding(nn.Module):
    ...
    def __init__(self, path_num, hop_num, feature_size, latent_dim):
        ...
        if hop_num == 3:
            ...
            raise Exception("Only support 3-hop or 4-hop or 5-hop metapaths, hop %d" % (hop_num))
...
class MCRec(nn.Module):
    ...
    def forward(self, user_input, item_input, metapath_inputs):
        ...
        if path_output is None:
            path_output = torch.zeros(user_input.shape[0] * user_input.shape[1], 1, self.latent_dim).to(self.device)
        else:
